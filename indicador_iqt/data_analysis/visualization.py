...
class Graficos:
    ...
    def plot_histograma_passageiros(self):
        ...
        sns.histplot(pd.to_numeric(self.df['qtpsg'], errors='coerce'), kde=True)
    ...
    def plot_tendencia_passageiros(self):
        ...
        # self.df['datai'] = pd.to_datetime(self.df['datai'], dayfirst=True)  # Certifique-se de ter uma coluna 'datai'
        df_grouped = self.df.groupby(self.df['datai'].dt.to_period('M'))['qtpsg'].sum()
        plt.figure(figsize=(12, 6))
    ...
    def plot_barras_empilhadas(self):
        ...
        # self.df['datai'] = pd.to_datetime(self.df['datai'], dayfirst=True)
        self.df['mes'] = self.df['datai'].dt.to_period('M')
    ...
    def plot_area_passageiros(self):
        ...
        # self.df['datai'] = pd.to_datetime(self.df['datai'], dayfirst=True)
        self.df_grouped = self.df.groupby([self.df['datai'].dt.to_period('M'), 'linha'])['qtpsg'].sum().unstack().fillna(0)
