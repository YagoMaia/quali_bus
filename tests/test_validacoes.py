...
from indicador_iqt.utils.models import (
    validate_dataframe, validate_gdf_city, validate_df_dados_linhas, 
    validate_df_frequencia, validate_df_pontualidade, 
    validate_df_cumprimento, validate_residencias, validate_pontos_onibus
)
...
def test_validate_dataframe_incorrect_types():
...
def test_validate_gdf_city():
    ...
    assert validate_gdf_city(gdf)

def test_validate_gdf_city_missing_columns():
    ...
    with pytest.raises(ValueError, match="gdf_city está faltando colunas"):
        validate_gdf_city(gdf)

def test_validate_df_dados_linhas():
    ...
    assert validate_df_dados_linhas(df)

def test_validate_df_frequencia():
    """Test frequência DataFrame validation."""
    df = pd.DataFrame({
        'hsstart': ['06:00:00', '07:00:00'],
        'hsstop': ['07:00:00', '08:00:00'],
        'datai': ['01/01/2024', '02/01/2024'],
        'sentido': [0, 1],
        'linha': ['L1', 'L2'],
        'qtpsg': [10, 20],
        'valor_jornada': [100, 200]
    })
    assert validate_df_frequencia(df)

def test_validate_df_pontualidade():
    ...
    assert validate_df_pontualidade(df)

def test_validate_df_cumprimento():
    ...
    assert validate_df_cumprimento(df)

def test_validate_residencias():
    ...
    assert validate_residencias(gdf)

def test_validate_pontos_onibus():
    ...
    assert validate_pontos_onibus(gdf)