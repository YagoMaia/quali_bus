...
def sample_residencias():
	...
	return pd.DataFrame({"longitude": [-43.9386, -43.9389, -43.9392], "latitude": [-19.9176, -19.9179, -19.9182], "ID": [1, 2, 3]})


@pytest.fixture
def sample_pontos_onibus():
	"""Fixture que cria um DataFrame de pontos de ônibus para teste."""
	return pd.DataFrame({"longitude": [-43.9387, -43.9390], "latitude": [-19.9177, -19.9180], "ID": [1, 2]})
...
def test_verificar_formato_coordenadas(associador, sample_residencias):
	...
	invalid_coords = pd.DataFrame({"longitude": [200, -200], "latitude": [100, -100]})
