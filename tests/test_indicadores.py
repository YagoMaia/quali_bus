...
def sample_frequencia_atendimento_pontuacao():
	...
	data = {
		"empresa": [1, 2, 3],
		"id_linha": ["4601", "4601", "4601"],
		"horario_inicio_jornada": ["06:07:57", "06:07:59", "06:10:59"],
		"horario_fim_jornada": ["06:10:57", "06:15:59", "06:19:59"],
...
def sample_pontualidade():
	...
	data = {
		...
		"descricao_trajeto": ["4601 - Rota Principal (ida)", "4601 - Rota Principal (volta)"],
		"chegada_planejada": ["05:34:26", "-"],
		"partida_real": ["00:42:00", "-"],
		"chegada_real": ["05:52:00", "-"],
	}
	return pd.DataFrame(data)
...
def test_carregar_dados_linha(calculator, sample_lines):
	...
	assert "id_linha" in loaded_lines.columns, "A coluna 'id_linha' deve estar presente"
	assert loaded_lines.geometry.geom_type.eq("LineString").all(), "Todas as geometrias devem ser LineStrings"
...
def test_carregar_cumprimento(calculator, sample_cumprimento):
	...
	assert "id_linha" in cumprimento.columns, "A coluna 'trajeto' deve estar presente"
	assert "km_executado" in cumprimento.columns, "A coluna 'cumprimento' deve estar presente"


def test_carregar_frequencia_atendimento_pontuacao(calculator, sample_frequencia_atendimento_pontuacao):
	"""
	Testa o método `carregar_frequencia_atendimento_pontuacao` para carregar frequência de atendimento.
	"""
	frequencia = calculator.carregar_frequencia_atendimento_pontuacao(sample_frequencia_atendimento_pontuacao)

	assert not frequencia.empty, "O DataFrame de frequência não deveria estar vazio"
	assert "id_linha" in frequencia.columns, "A coluna 'id_linha' deve estar presente"
	assert "frequencia_atendimento_pontuacao" in frequencia.columns, "A coluna 'frequencia_atendimento_pontuacao' deve estar presente"
...
def test_calcular_pontualidade(calculator, sample_pontualidade):
	...
	assert "id_linha" in pontualidade.columns, "A coluna 'id_linha' deve estar presente"
	assert "pontualidade" in pontualidade.columns, "A coluna 'pontualidade' deve estar presente"
...
def test_cumprimento_itinerario(calculator, sample_cumprimento):
	...
	assert not cumprimento.empty, "O DataFrame de cumprimento não deveria estar vazio"
	assert "id_linha" in cumprimento.columns, "A coluna 'id_linha' deve estar presente"
	assert "km_executado" in cumprimento.columns, "A coluna 'km_executado' deve estar presente"
...
def test_calcular_iqt(calculator):
	...
	assert isinstance(iqt, float), "O resultado do IQT deve ser um número de ponto flutuante"


def test_carregar_dados(calculator, sample_lines, sample_frequencia_atendimento_pontuacao, sample_pontualidade, sample_cumprimento):
	"""
	Testa o método `carregar_dados` com todos os DataFrames.
	"""
	calculator.carregar_dados(sample_lines, sample_frequencia_atendimento_pontuacao, sample_pontualidade, sample_cumprimento)
